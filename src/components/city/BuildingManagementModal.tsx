...
export default function BuildingManagementModal({
  ...
  const getBuildingSpecificContent = () => {
    ...
    switch (buildingData.slug) {
      ...
      case 'academy':
        return <Academy level={props.level} />;
      case 'smith':
        ...
        <div className="p-6 overflow-y-auto max-h-[70vh] bg-forest-dark">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gold mb-3">Building Information</h3>
                <p className="text-gold-light mb-4">{buildingData.description}</p>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-forest-light p-4 rounded-lg border border-forest">
                  <h4 className="font-semibold text-gold mb-3">Upgrade to Level {building.level + 1}</h4>
                  
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gold-light">Food:</span>
                      <span className={`text-sm ${cityResources.food >= upgradeCosts.food ? 'text-gold-light' : 'text-red-400'}`}>
                        {cityResources.food} / {upgradeCosts.food}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gold-light">Wood:</span>
                      <span className={`text-sm ${cityResources.wood >= upgradeCosts.wood ? 'text-gold-light' : 'text-red-400'}`}>
                        {cityResources.wood} / {upgradeCosts.wood}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gold-light">Stone:</span>
                      <span className={`text-sm ${cityResources.stone >= upgradeCosts.stone ? 'text-gold-light' : 'text-red-400'}`}>
                        {cityResources.stone} / {upgradeCosts.stone}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gold-light">Ore:</span>
                      <span className={`text-sm ${cityResources.ore >= upgradeCosts.ore ? 'text-gold-light' : 'text-red-400'}`}>
                        {cityResources.ore} / {upgradeCosts.ore}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gold-light">Gold:</span>
                      <span className={`text-sm ${cityResources.gold >= upgradeCosts.gold ? 'text-gold-light' : 'text-red-400'}`}>
                        {cityResources.gold} / {upgradeCosts.gold}
                      </span>
                    </div>
                  </div>
